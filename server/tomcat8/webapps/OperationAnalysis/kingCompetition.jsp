...
<%
	...
	JSONArray kingCompetitionData = new JSONArray();
	String dataStr = "";
	if (data != null && !data.isNull("dataList")) {
		kingCompetitionData = (JSONArray) data.getJSONArray("dataList");
		if (kingCompetitionData.length() > 0) {
			JSONObject obj = kingCompetitionData.getJSONObject(kingCompetitionData.length() - 1);
...
<script type="text/javascript" language="javascript">
	$(function() {
		...
		$('#chart_1').highcharts({
			...
			xAxis : {
				categories : ['0次', '1-5次', '6-10次', '11-15次', '16-20次', '21-25次', '26-30次']
			},
			yAxis : {
				title : {
					text : '百分比（单位：%）'		
...
<body data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-30" data-genuitec-path="/OperationAnalysis/WebRoot/kingCompetition.jsp">
	<div class="container" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-30" data-genuitec-path="/OperationAnalysis/WebRoot/kingCompetition.jsp">
		<div class="header">
			...
			<div class="content">
				<div class="content_top">
					<form action="<%=path %>/behaviorAnalysis" method="post">
						<input type="hidden" name="optType" value="<%= OptTypeConstant.BEHAVIOR_ANALYSIS_5%>" />
				...
				<div class="content_panel">
					...
					<div class="content_panel_content">
						...
						<table id="table_1" class="display" cellspacing="0" width="100%">
							...
							<tbody>
								<%
									for (int i = 0; i < kingCompetitionData.length(); i++) {
										JSONObject obj = kingCompetitionData.getJSONObject(i);
