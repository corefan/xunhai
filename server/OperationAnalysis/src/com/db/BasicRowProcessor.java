...
public class BasicRowProcessor {
	 ...
	 * @throws SQLException
	 *             if a database access error occurs
	 * @return A <code>List</code> of beans with the given type in the order they were returned by the
	 *         <code>ResultSet</code>.
	 */
	 ...
	 * case insensitive column names as keys. Calls to <code>map.get("COL")</code> and <code>map.get("col")</code>
	 ...
	 * implementation because databases don't consistently handle the casing of column names.
	 * 
	 * <p>
	 * The keys are stored as they are given [BUG #DBUTILS-34], so we maintain an internal mapping from lowercase keys
	 * to the real keys in order to achieve the case insensitive lookup.
	 * 
	 * <p>
	 * Note: This implementation does not allow <tt>null</tt> for key, whereas {@link HashMap} does, because of the
	 * code:
	 * 
	 * <pre>
	...
	private static class CaseInsensitiveHashMap extends HashMap<String, Object> {
		 ...
		 * 
		 * <p>
		 * Any query operation using the key ({@link #get(Object)}, {@link #containsKey(Object)}) is done in three
		 * steps:
		 * <ul>
		...
		public Object put(String key, Object value) {
			/*
			 * In order to keep the map and lowerCaseMap synchronized, we have to remove the old mapping before putting
			 * the new one. Indeed, oldKey and key are not necessaliry equals. (That's why we call super.remove(oldKey)
			 * and not just super.put(key, value))
			 */
