package com.cc.app.player {
    ...
    public class PlayerMediator extends Mediator {
		...
		public function addCMDListener():void{
			...
			GameCCSocket.instance.addCmdListener(OptTypeConstant.PLAYER_8, onPlayerConsumeCheckBack);
			GameCCSocket.instance.addCmdListener(OptTypeConstant.PLAYER_9, onPlayerPayCheckBack);
			...
			GameCCSocket.instance.addCmdListener(OptTypeConstant.PLAYER_LOG_30, onGoddessLogBack);
			GameCCSocket.instance.addCmdListener(OptTypeConstant.PLAYER_LOG_31, onSweetyLogBack);
			GameCCSocket.instance.addCmdListener(OptTypeConstant.PLAYER_LOG_32, onMountLogBack);
			GameCCSocket.instance.addCmdListener(OptTypeConstant.PLAYER_LOG_33, onPetLogBack);
			GameCCSocket.instance.addCmdListener(OptTypeConstant.PLAYER_LOG_34, onMagicLogBack);
			GameCCSocket.instance.addCmdListener(OptTypeConstant.PLAYER_LOG_35, onEquipmentLogBack);
		...
		public function removeCMDListener():void {
			...
			GameCCSocket.instance.removeCmdListener(OptTypeConstant.PLAYER_8, onPlayerConsumeCheckBack);
			GameCCSocket.instance.removeCmdListener(OptTypeConstant.PLAYER_9, onPlayerPayCheckBack);
			...
			GameCCSocket.instance.removeCmdListener(OptTypeConstant.PLAYER_LOG_30, onGoddessLogBack);
			GameCCSocket.instance.removeCmdListener(OptTypeConstant.PLAYER_LOG_31, onSweetyLogBack);
			GameCCSocket.instance.removeCmdListener(OptTypeConstant.PLAYER_LOG_32, onMountLogBack);
			GameCCSocket.instance.removeCmdListener(OptTypeConstant.PLAYER_LOG_33, onPetLogBack);
			GameCCSocket.instance.removeCmdListener(OptTypeConstant.PLAYER_LOG_34, onMagicLogBack);
			GameCCSocket.instance.removeCmdListener(OptTypeConstant.PLAYER_LOG_35, onEquipmentLogBack);
		}
...
//                PlayerModel.instance.site = result.gameSite;
         ...
         * 查询玩家消费记录返回
         * */
        private function onPlayerConsumeCheckBack(data:ByteArray, mid:int):void {
            if (data) {
				var str:String = data.readUTFBytes(data.length);
				str = StringUtil.trim(str);
				var result:Object = JSON.parse(str);
                player.setPlayerConsumeInfo(result);
            } else {
                MessageUtil.showFaultMessage("玩家无消费记录");
            }
        }
		 ...
		 * 查询玩家充值记录返回
		 * */
		private function onPlayerPayCheckBack(data:ByteArray, mid:int):void {
			if (data) {
				var str:String = data.readUTFBytes(data.length);
				str = StringUtil.trim(str);
				var result:Object = JSON.parse(str);
				player.setPlayerPayInfo(result);
			} else {
				MessageUtil.showFaultMessage("玩家无充值记录");
			}
		}
         ...
         * 查询玩家女神日志记录返回
         * */
        private function onGoddessLogBack(data:ByteArray, mid:int):void {
            if (data) {
				var str:String = data.readUTFBytes(data.length);
				str = StringUtil.trim(str);
				var result:Object = JSON.parse(str);
                player.setGoddessLogInfo(result);
            } else {
                MessageUtil.showFaultMessage("玩家无女神日志记录");
            }
        }
        /**
         * 查询玩家翅膀日志记录返回
         * */
        private function onSweetyLogBack(data:ByteArray, mid:int):void {
            if (data) {
				var str:String = data.readUTFBytes(data.length);
				str = StringUtil.trim(str);
				var result:Object = JSON.parse(str);
                player.setSweetyLogInfo(result);
            } else {
                MessageUtil.showFaultMessage("玩家无翅膀记录");
            }
        }
        /**
         * 查询玩家坐骑日志记录返回
         * */
        private function onMountLogBack(data:ByteArray, mid:int):void {
            if (data) {
				var str:String = data.readUTFBytes(data.length);
				str = StringUtil.trim(str);
				var result:Object = JSON.parse(str);
                player.setMountLogInfo(result);
            } else {
                MessageUtil.showFaultMessage("玩家无坐骑记录");
            }
        }
        /**
         * 查询玩家精灵日志记录返回
         * */
        private function onPetLogBack(data:ByteArray, mid:int):void {
            if (data) {
				var str:String = data.readUTFBytes(data.length);
				str = StringUtil.trim(str);
				var result:Object = JSON.parse(str);
                player.setPetLogInfo(result);
            } else {
                MessageUtil.showFaultMessage("玩家无精灵记录");
            }
        }
        /**
         * 查询玩家魔法阵日志记录返回
         * */
        private function onMagicLogBack(data:ByteArray, mid:int):void {
            if (data) {
				var str:String = data.readUTFBytes(data.length);
				str = StringUtil.trim(str);
				var result:Object = JSON.parse(str);
                player.setMagicLogInfo(result);
            } else {
                MessageUtil.showFaultMessage("玩家无魔法阵记录");
            }
        }
        /**
         * 查询玩家装备附魔记录返回
         * */
        private function onEquipmentLogBack(data:ByteArray, mid:int):void {
            if (data) {
				var str:String = data.readUTFBytes(data.length);
				str = StringUtil.trim(str);
				var result:Object = JSON.parse(str);
                player.setEquipmentFmLogInfo(result);
            } else {
                MessageUtil.showFaultMessage("玩家无装备附魔记录");
            }
        }
