...
public class DisruptorServer {
	...
	private RingBuffer<GameMessage> ringBuffer;
	
	private Disruptor<GameMessage> disruptor;
	...
	public void start() {
		...
		disruptor = new Disruptor<GameMessage>(new EventFactory<GameMessage>() {
			@Override
			public GameMessage newInstance() {
				return new GameMessage();
			}
		}, ThreadPoolService.DISRUPTOR_BUFF_SIZE, executor, ProducerType.MULTI, new BlockingWaitStrategy());

		WorkHandler<GameMessage>[] workHandlers = new DisruptorMsgHandler[ThreadPoolService.DISRUPTOR_CONSUMER_THREAD_NUM];
	...
	public RingBuffer<GameMessage> getRingBuffer() {
		return ringBuffer;
	}
}
