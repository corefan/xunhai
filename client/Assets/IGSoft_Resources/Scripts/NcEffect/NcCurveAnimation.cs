...
public class NcCurveAnimation : NcEffectAniBehaviour
{
    ...
    protected float m_fDelayTimeOnStart = 0;
    public float m_fDurationTime = 0.6f;
    protected float m_fDurationTimeOnStart = 0;
    ...
    protected bool m_restart = false;
    ...
    public override string CheckProperty()
    {
        ...
        foreach (NcInfoCurve curveInfo in m_CurveInfoList)
        {
            ...
            if (curveInfo.m_ApplyType == NcInfoCurve.APPLY_TYPE.MATERIAL_COLOR)
            {
                ...
                if (curveInfo.m_bRecursively == false && (ren == null || Ng_GetMaterialColorName(ren.sharedMaterial) == null))
    ...
    public override int GetAnimationState()
    {
        if (enabled == false || IsActive(gameObject) == false)
            return -1;
        // 		Log.info(m_fStartTime);
        // 		Log.info(IsEndAnimation());
    ...
    void Awake()
    {
        // 		Log.info("NcCurveAnimation.Awake " + transform.parent.name);
    }

    void Start()
    {
        if (m_restart) return;
        //		Log.info("NcCurveAnimation.Start " + transform.parent.name);
        m_fStartTime = GetEngineTime();
        InitAnimation();
    ...
    public bool isDelay()
    {
        return GetEngineTime() <= m_fStartTime + m_fDelayTime;
    }
    void LateUpdate()
    {
        // 		Log.info("NcCurveAnimation.Update " + transform.parent.name);
        if (m_fStartTime == 0)
            return;
        ...
        foreach (NcInfoCurve curveInfo in m_CurveInfoList)
        {
            ...
            switch (curveInfo.m_ApplyType)
            {
                ...
                case NcInfoCurve.APPLY_TYPE.MATERIAL_COLOR:
                    {
                        ...
                        {
                            startMaterialColor = new Dictionary<Renderer, ColorData>();
                        }
                        if (curveInfo.m_bRecursively)
                        {
                            ...
                            for (int n = 0; n < m_ChildRenderers.Length; n++)
                            {
                                ...
                                if (m_ChildColorNames[n] != null)
                                {
                                    ...
                                    if (!startMaterialColor.ContainsKey(ren))
                                    {
                                        startMaterialColor[ren] = new ColorData(m_ChildColorNames[n], curveInfo.m_ChildOriginalColorValues[n]);
                                    }
                        ...
                        else
                        {
                            ...
                            if (ren != null)
                            {
                                m_ColorName = Ng_GetMaterialColorName(ren.sharedMaterial);
                                if (m_ColorName != null)
                                {
                                    curveInfo.m_OriginalValue = ren.sharedMaterial.GetColor(m_ColorName);
                                    if (!startMaterialColor.ContainsKey(ren))
                                    {
                                        startMaterialColor[ren] = new ColorData(m_ColorName, curveInfo.m_OriginalValue);
                                    }
                                }
                ...
                case NcInfoCurve.APPLY_TYPE.MESH_COLOR:
                    {
                        if (startMeshColor == null)
                        {
                            startMeshColor = new Dictionary<MeshFilter, ColorData>();
                        }
                        ...
                        if (curveInfo.m_bRecursively)
                        {
                            ...
                            {
                                MeshFilter mf = m_ChildMeshFilters[arrayIndex];
                                ChangeMeshColor(mf, tarColor);
                                if (!startMeshColor.ContainsKey(mf))
                                {
                                    startMeshColor[mf] = new ColorData(null, tarColor);
                                }
                            }
                        }
                        else
                        {
                            ...
                            if (!startMeshColor.ContainsKey(m_MainMeshFilter))
                            {
                                startMeshColor[m_MainMeshFilter] = new ColorData(null, tarColor);
                            }
                        }
                        break;
                    }
    ...
    public bool CheckInvalidOption(int nSrcIndex)
    {
        ...
        // 					{
        // 						Log.info(GetHelpString("DUPLICATE", string.Format("CurveIndex [{0}], TypeValue[{1}]", nIndex.ToString(), m_CurveInfoList[nIndex].GetValueName(n))));
        // 						bDup = true;
        // 					}
        // 			}
        // 		}
    ...
    private Dictionary<Renderer, ColorData> startMaterialColor;
    private Dictionary<MeshFilter, ColorData> startMeshColor;

    public override void RecordStartState()
    {
        base.RecordStartState();
        m_fDelayTimeOnStart = m_fDelayTime;
        m_fDurationTimeOnStart = m_fDurationTime;
        //
    ...
    public override void ResetToStartState()
    {
        ...
        m_fDelayTime = m_fDelayTimeOnStart;
        m_fDurationTime = m_fDurationTimeOnStart;
        // 重置
        ...
        if (startMaterialColor != null)
        {
            foreach (KeyValuePair<Renderer, ColorData> kvp in startMaterialColor)
            {
                kvp.Key.material.SetColor(kvp.Value.name, kvp.Value.color);
            }
        }
        if (startMeshColor != null)
        {
            foreach (KeyValuePair<MeshFilter, ColorData> kvp in startMeshColor)
            {
                ChangeMeshColor(kvp.Key, kvp.Value.color);
            }
        }
    }
    class ColorData
