...
convertRad2Angle = 180 / math.pi -- 弧度转角度
...
function Mathf.IsNan(number)
 ...
 --出坐标的角度计算（后者在前者的什么方向）
function Mathf.AngleCompute(x1, y1, x2, y2)
    local temp_a = x2 - x1
    local temp_b = y2 - y1
    local temp_c = Mathf.Sqrt(temp_a * temp_a + temp_b * temp_b)
    local temp_angle = Mathf.Abs(Mathf.Asin(temp_b / temp_c) * 180 / Mathf.PI)
    if x2 > x1 then
        if y2 > y1 then
